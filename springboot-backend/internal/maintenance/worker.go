...
func (w *Worker) resetMonthlyTraffic(ctx context.Context, now time.Time) (bool, error) {
	...
	defer tx.Rollback()
	claimed, err := claimRun(ctx, tx, "monthly_traffic_reset", period, now.UnixMilli())
	if err != nil || !claimed {
		return false, err
	}
	day := now.Day()
	lastDay := time.Date(now.Year(), now.Month()+1, 0, 0, 0, 0, 0, now.Location()).Day()
	result, err := tx.ExecContext(ctx, `UPDATE users SET ingress_bytes=0,egress_bytes=0,updated_at=? WHERE role<>'admin' AND flow_reset_day>0 AND (flow_reset_day=? OR (?=? AND flow_reset_day>?))`, now.UnixMilli(), day, day, lastDay, lastDay)
	if err != nil {
		return false, fmt.Errorf("reset monthly user traffic: %w", err)
	}
	userCount, err := result.RowsAffected()
	if err != nil {
		return false, err
	}
	result, err = tx.ExecContext(ctx, `UPDATE user_tunnels SET ingress_bytes=0,egress_bytes=0,updated_at=? WHERE flow_reset_day>0 AND (flow_reset_day=? OR (?=? AND flow_reset_day>?))`, now.UnixMilli(), day, day, lastDay, lastDay)
	if err != nil {
		return false, fmt.Errorf("reset monthly user tunnel traffic: %w", err)
	}
...
func (w *Worker) recordHourlyStatistics(ctx context.Context, now time.Time) error {
	...
	defer tx.Rollback()
	claimed, err := claimRun(ctx, tx, "hourly_traffic_statistics", period, now.UnixMilli())
	if err != nil || !claimed {
		return err
	}
