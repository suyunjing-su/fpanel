...
import type { EndpointGroup, RouteRuleSet } from "@/types";
...
interface Forward {
  ...
  strategy: string;
  endpointGroupId?: number | null;
  routeRuleSetId?: number | null;
  maxConnections?: number;
  maxConnectionsPerIp?: number;
  sourceRanges?: string;
  sourceWhitelist?: number;
...
export default function ForwardPage() {
  ...
  const [routeRuleSets, setRouteRuleSets] = useState<RouteRuleSet[]>([]);
  ...
  const loadData = async (lod = true) => {
    setLoading(lod);
    try {
      const [forwardsRes, tunnelsRes, endpointGroupsRes, routeRuleSetsRes] =
        await Promise.all([
          getForwardList(),
          userTunnel(),
          admin
            ...
            : Promise.resolve({
                code: 0,
                msg: "",
                data: [] as EndpointGroup[],
              }),
          admin
            ? getRouteRuleSets()
            : Promise.resolve({ code: 0, msg: "", data: [] as RouteRuleSet[] }),
        ]);
      ...
      if (endpointGroupsRes.code === 0) {
        setEndpointGroups(endpointGroupsRes.data || []);
      } else {
        toast.error(endpointGroupsRes.msg || "获取端点组失败");
      }
      if (routeRuleSetsRes.code === 0) {
        setRouteRuleSets(routeRuleSetsRes.data || []);
      } else {
        toast.error(routeRuleSetsRes.msg || "获取路由规则集失败");
      }
    } catch (error) {
  ...
  const batchSelection = useBatchDeleteSelection<Forward>({
    items: forwards,
    entityLabel: "转发",
    batchDeleteApi: batchDeleteForwards,
    reloadData: async () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.remoteAddr.trim() && !form.endpointGroupId) {
      newErrors.remoteAddr = "请输入远程地址或选择端点组";
  ...
  const handleEdit = (forward: Forward) => {
    ...
    setForm({
      ...
      endpointGroupId: forward.endpointGroupId ?? null,
      routeRuleSetId: forward.routeRuleSetId ?? null,
      maxConnections: forward.maxConnections || 0,
      maxConnectionsPerIp: forward.maxConnectionsPerIp || 0,
      sourceRanges: forward.sourceRanges || "",
      sourceWhitelist: forward.sourceWhitelist || 0,
      proxyProtocolReceive: forward.proxyProtocolReceive || 0,
      proxyProtocolSend: forward.proxyProtocolSend || 0,
    });
    setErrors({});
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const addressCount = processedRemoteAddr
        ? processedRemoteAddr.split(",").length
        : 0;
      const payload = {
        userId: form.userId,
        name: form.name,
        ...
        strategy:
          addressCount > 1 || form.endpointGroupId ? form.strategy : "fifo",
        endpointGroupId: admin ? form.endpointGroupId : null,
        routeRuleSetId: admin ? form.routeRuleSetId : null,
        maxConnections: admin ? form.maxConnections : 0,
        maxConnectionsPerIp: admin ? form.maxConnectionsPerIp : 0,
        sourceRanges: admin ? form.sourceRanges : "",
        sourceWhitelist: admin ? form.sourceWhitelist : 0,
        proxyProtocolReceive: admin ? form.proxyProtocolReceive : 0,
        proxyProtocolSend: admin ? form.proxyProtocolSend : 0,
      };
      const res = isEdit
  ...
  const normalizeAddressForDisplay = (
    rawAddress: string,
    fallbackPort?: number,
  ): string => {
    ...
    if (bracketedWithoutPort) {
      return fallbackPort
        ? `[${bracketedWithoutPort[1]}]:${fallbackPort}`
        : value;
    }
  ...
  const formatInAddress = (ipString: string, port: number): string => {
    ...
    const normalized = items.map((item) =>
      normalizeAddressForDisplay(item, port || undefined),
    );
          ...
          setImportResults((prev) => [
            {
              ...
              success: false,
              message:
                "目标地址格式错误，应为 地址:端口 或 [IPv6]:端口 格式，多个地址用逗号分隔",
            },
            ...prev,
          ]);
          continue;
  ...
  // 根据排序顺序获取转发列表
  const getSortedForwards = (
    sourceForwards: Forward[] = filteredForwards,
  ): Forward[] => {
    // 确保 forwards 数组存在且有效
          ...
          <div className="flex justify-between items-start w-full">
            <div className="flex items-start gap-2 flex-1 min-w-0">
              <input
                ...
                checked={batchSelection.selectedIds.includes(forward.id)}
                onChange={() => batchSelection.toggleItemSelection(forward.id)}
  ...
  const runningCount = filteredForwards.filter(
    (item) => item.serviceRunning,
  ).length;
  const abnormalCount = filteredForwards.filter(
    (item) => item.status === 2,
  ).length;
      ...
      <div className="panel-shell p-3 lg:p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm panel-muted">
            当前显示 {filteredForwards.length} 条转发记录
            {viewMode === "direct" && hasActiveFilter
              ? "（筛选状态下禁用拖拽排序）"
              : ""}
          </div>
          ...
          <div className="space-y-6">
            {userGroups.map((userGroup) => (
              <Card
                ...
                className="panel-shell w-full overflow-hidden"
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between w-full min-w-0">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      {(() => {
                        const userForwardIds = getUserForwardIds(
                          userGroup.userId,
                        );
                        const allSelected =
                          batchSelection.areAllItemsSelected(userForwardIds);
                        return (
                          <input
                            type="checkbox"
                            ...
                            checked={allSelected}
                            onChange={() =>
                              batchSelection.setItemsSelected(
                                userForwardIds,
                                !allSelected,
                              )
                            }
                  ...
                  {admin && (
                    <Accordion variant="bordered">
                      <AccordionItem
                        ...
                        aria-label="高级运行策略"
                        title="高级运行策略"
                        subtitle="端点路由、连接限制、来源准入与 Proxy Protocol"
                      >
                        <div className="space-y-4 pb-3">
                          <div className="grid md:grid-cols-2 gap-3">
                            <Select
                              ...
                              onSelectionChange={(keys) => {
                                ...
                                setForm((prev) => ({
                                  ...prev,
                                  endpointGroupId: selected
                                    ? Number(selected)
                                    : null,
                                  routeRuleSetId: selected
                                    ? prev.routeRuleSetId
                                    : null,
                                }));
                              }}
                            ...
                            <Select
                              label="路由规则集"
                              placeholder="不使用路由规则"
                              isDisabled={!form.endpointGroupId}
                              selectedKeys={
                                form.routeRuleSetId
                                  ? [String(form.routeRuleSetId)]
                                  : []
                              }
                              onSelectionChange={(keys) => {
                                const selected = Array.from(keys)[0];
                            ...
                            <Input
                              ...
                              min={0}
                              label="单 IP 连接上限"
                              value={String(form.maxConnectionsPerIp)}
                              onValueChange={(value) =>
                                setForm((prev) => ({
                                  ...prev,
                            ...
                            <Input
                              ...
                              onValueChange={(interfaceName) =>
                                setForm((prev) => ({ ...prev, interfaceName }))
                              }
                              variant="bordered"
                            />
                            <Select
                              label="来源规则模式"
                              selectedKeys={[String(form.sourceWhitelist)]}
                              onSelectionChange={(keys) => {
                                const selected = Array.from(keys)[0];
                                if (selected)
                                  setForm((prev) => ({
                                    ...prev,
                                    sourceWhitelist: Number(selected),
                                  }));
                              }}
                              variant="bordered"
                            >
                            ...
                            <Select
                              ...
                              selectedKeys={[String(form.proxyProtocolReceive)]}
                              onSelectionChange={(keys) => {
                                const selected = Array.from(keys)[0];
                                if (selected)
                                  setForm((prev) => ({
                                    ...prev,
                                    proxyProtocolReceive: Number(selected),
                                  }));
                              }}
                              variant="bordered"
                            >
                              <SelectItem key="0">关闭</SelectItem>
                              <SelectItem key="1">v1</SelectItem>
                              <SelectItem key="2">v2</SelectItem>
                            </Select>
                            <Select
                              label="发送 Proxy Protocol"
                              selectedKeys={[String(form.proxyProtocolSend)]}
                              onSelectionChange={(keys) => {
                                const selected = Array.from(keys)[0];
                                if (selected)
                                  setForm((prev) => ({
                                    ...prev,
                                    proxyProtocolSend: Number(selected),
                                  }));
                              }}
                              variant="bordered"
                            >
                          ...
                          <Textarea
                            ...
                            onValueChange={(sourceRanges) =>
                              setForm((prev) => ({ ...prev, sourceRanges }))
                            }
                            variant="bordered"
                            minRows={2}
                          />
                          <Alert color="primary" variant="flat">
                            使用端点组时远程地址可留空；路由规则集中的端点必须属于所选端点组。
                          </Alert>
                        </div>
                      </AccordionItem>
                    </Accordion>
